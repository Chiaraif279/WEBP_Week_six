...
showPlayerInput(onStart: (name: string) => void) { //user kann namen eingeben am anfang
  ...
  showFeedback(isCorrect: boolean, correctAnswer: string, onNext: () => void) {
    this.container.innerHTML += `
      <div class="mt-3">
  ...
  showFinalResult(player: Player, onRestart: () => void) {
  this.container.innerHTML = `
    <h2>Ergebnis</h2>
 ...
 showLeaderboard(currentPlayer?: Player) {
  let leaderboard: Player[] = JSON.parse(localStorage.getItem("leaderboard") || "[]");
